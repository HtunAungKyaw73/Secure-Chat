...
import { Send, User as UserIcon, MessageSquare, LogOut } from "lucide-react";
...
export default function ChatApp() {
  ...
  useEffect(() => {
    if (isJoined) {
      ...
      socket.on("user_joined", (newUsername: string) => {
        // We could also show a toast or a system message
            ...
            <div className="flex flex-col gap-2">
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-3 bg-zinc-950 border border-zinc-800 rounded-xl focus:outline-none focus:ring-2 focus:ring-violet-500/50 focus:border-violet-500 transition-all text-white placeholder:text-zinc-600"
                  maxLength={20}
                  autoFocus
                />
              </div>
  ...
  return (
    <div className="min-h-screen bg-zinc-950 flex flex-col items-center justify-center p-0 sm:p-4 md:p-8 font-sans">
      <motion.div
      ...
      >
        ...
        <header className="px-6 py-4 border-b border-zinc-800/50 bg-zinc-900/80 backdrop-blur-md flex items-center justify-between z-10 shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-semibold text-zinc-100 leading-tight">VibeChat Room</h2>
              <div className="flex items-center gap-1.5">
                <span className="w-2 h-2 rounded-full bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.5)]"></span>
                <span className="text-xs text-zinc-400 font-medium">Connected as <span className="text-zinc-300">{username}</span></span>
              </div>
            </div>
          </div>
          <button
            ...
            {messages.map((msg) => {
              ...
              if (isSystem) {
                return (
                  ...
                  >
                    <span className="px-3 py-1 bg-zinc-800/50 text-zinc-400 text-xs rounded-full border border-zinc-700/50">
                      {msg.text}
                    </span>
                  </motion.div>
                );
              ...
              return (
                ...
                >
                  <span className={`text-[11px] font-medium mb-1 px-1 ${isMe ? "text-violet-400" : "text-zinc-400"}`}>
        ...
        {/* Input area */}
        <div className="p-4 sm:p-6 bg-zinc-900/80 border-t border-zinc-800/50 backdrop-blur-md shrink-0">
          <form onSubmit={handleSendMessage} className="relative flex items-center max-w-3xl mx-auto">
            <input
              type="text"
              value={newMessage}
              onChange={(e) => setNewMessage(e.target.value)}
              placeholder="Message VibeChat..."
              className="w-full bg-zinc-950 border border-zinc-800/80 rounded-full pl-5 pr-14 py-3.5 text-sm text-zinc-100 focus:outline-none focus:ring-2 focus:ring-violet-500/50 focus:border-violet-500 transition-all shadow-inner placeholder:text-zinc-500"
              autoComplete="off"
            />
            <button
              type="submit"
              disabled={!newMessage.trim()}
              className="absolute right-1.5 p-2 bg-violet-600 hover:bg-violet-500 disabled:bg-zinc-800 disabled:text-zinc-600 text-white rounded-full transition-colors flex items-center justify-center shadow-lg"
            >
